(function(){
				...
				$scope.showAlert = function(ev, premiosUrl) {
					...
					$mdDialog.show({
						controller: function($mdDialog) {
							...
							urlScrap="http://loto.diegobergua.com/scrap_premios.php?game="+game+"&urlId="+urlId+"&year="+year;
						...
						templateUrl: 'premios.alert.html',
	...
	app.directive('resultBoletos', function(){
		return{
			restrict: 'A',
			templateUrl: 'result-boletos.html',
	...
	app.factory('getCombis', ["$http", function($http) {
		...
		return function(lotoVar, miFecha){
			if(miFecha){
				...
				urlScrap="http://loto.diegobergua.com/scrap_numeros.php?game="+game+"&d="+ resF[0]+"&m="+ resF[1]+"&a="+ resF[2];
			}else{
				urlScrap="http://loto.diegobergua.com/scrap_numeros.php?game="+game;
			}
			$http.get(urlScrap).success(function(data){
	...
	app.factory('getBoletos', ["$http", "$cookies", function($http, $cookies) {
		return function(boletosVar){
			kw = $cookies.get('kw');
			pin = $cookies.get('pin');
			$http.get("http://loto.diegobergua.com/read_json.php?kw="+kw+"&pin="+pin).success(function(data){
				boletosVar.boletos=[];
	...
	app.controller('LotoController', ["$scope", "$http", "$filter", "getCombis", "toggleSideBar", "$cookies", "getBoletos", function ($scope, $http, $filter, getCombis, toggleSideBar, $cookies, getBoletos) {
		...
		this.guardarBoleto = function(boletoTemp){
			...
			var request = $http({
				method: "post",
				url: "http://loto.diegobergua.com/save_json.php?kw="+loto.kw+"&pin="+loto.pin,
				data: boletoTemp,
		...
		this.borrarBoleto = function(boletoId){
			loto.kw = $cookies.get('kw');
			loto.pin = $cookies.get('pin');
			$http.get("http://loto.diegobergua.com/update_json.php?kw="+kw+"&pin="+pin+"&boletoId="+boletoId).success(function(data){
				loto.boletos = getBoletos(loto);
